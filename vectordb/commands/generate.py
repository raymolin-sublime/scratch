...
def execute(args):
    ...
    embeddings = model.encode(
        texts, batch_size=args.batch_size, show_progress_bar=True, convert_to_numpy=True
    )
...
def register_generate_command(subparsers):
    ...
    parser = subparsers.add_parser(
        "generate", help="Generate random text embeddings and save to HDF5"
    )
    parser.add_argument(
        "-n",
        "--num-vectors",
    ...
    parser.add_argument(
        "-o", "--output", type=str, required=True, help="Output path for the HDF5 file"
    )
    parser.add_argument(
        "--batch-size",
        type=int,
